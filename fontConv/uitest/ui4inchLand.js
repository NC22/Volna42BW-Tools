function testUI4in2Land() {  
       ...
       // handler.rotate = 90;
        ...
        var baseX = localWidth / 2;
        var theight = 28;
        var twidthTitle = 90;
        
        handler.drawString(baseX + 20, theight - 20, 'дом', fontData_cbold17x17, false);
        handler.drawString(baseX + 20, theight - 20 + 70, 'влажность', fontData_cbold17x17, false);
        handler.drawString(baseX + 20, theight + 70, '50%', fontData_cbold44x44, false);
        
        var twidth = handler.drawString(baseX + 10, theight, '12.23', fontData_cbold44x44, false);
        
        handler.renderImageArrayToBuffer(c, baseX + 10 + twidth, theight + 4, 39, 43, false, false, false);
        
        twidth += 39;
        
        handler.renderImageArrayToBuffer(temp, baseX + 16 + twidth, theight - 20, 15, 68, false, false, false);
        
        handler.colorMode = monitor.colors.ALT1;       
        handler.renderImageArrayToBuffer(tempMeter, baseX +  16 + twidth, theight - 20, 15, 68, false, false, true, false, 0, 0, 0, 68);            
        handler.colorMode = monitor.colors.BLACK;
        ...
        if (street) {
        handler.bufferFillRect(localWidth / 2, localHeight / 2, localWidth / 2, 2, false);
        
        theight = localHeight / 2 + 26;
        
        handler.drawString(baseX + 20, theight - 20, 'улица', fontData_cbold17x17, false);
        handler.drawString(baseX + 20, theight - 20 + 70, 'влажность', fontData_cbold17x17, false);
        handler.drawString(baseX + 20, theight + 70, '100%', fontData_cbold44x44, false);
        ...
        handler.renderImageArrayToBuffer(tempMeter, baseX + 16 + twidth, theight - 20, 15, 68, false, false, true, false, 0, 0, 55, 68);
        ...
        theight = localHeight - 125 - 80;
        
  
        var dateWidth = handler.drawStringGetWidth('24 декабря, вт', fontData_cbold17x17);
        baseX = 0;
        if (dateWidth < localWidth / 2) {
           baseX = Math.ceil(((localWidth / 2) - dateWidth)  / 2);
        }
        
        handler.drawString(baseX + Math.ceil((dateWidth - handler.drawStringGetWidth('20:15', fontData_cbold44x44)) / 2), theight, '20:15', fontData_cbold44x44, true);
        
        theight += 44;
		
		var testDate = 'вт, 03.12.' + new Date().getFullYear();
        handler.drawString(baseX + Math.ceil((dateWidth - handler.drawStringGetWidth(testDate, fontData_cbold17x17)) / 2), theight, testDate, fontData_cbold17x17, true);
        theight += 17;
        
        // image icon
        
        baseX = Math.ceil(((localWidth / 2) - 115)  / 2);
        handler.renderImageArrayToBuffer(cat115x125, baseX, localHeight - 125 - 2, 115, 125, false, false, true, false, 0, 0, 0, 0, false);
